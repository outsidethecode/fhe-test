...
fn u8_32_array_to_u16_16_array(arr: &[u8; 32]) -> [u16; 16] {
    let mut result: [u16; 16] = [0; 16];

    for i in 0..16 {
        let sub_arr = &arr[i*2 .. (i+1)*2];
...
fn calc_slice_value(byte_array: &[u8]) -> u16 {
...
fn calc_value(array: [u16; 16]) -> BigUint {
    let mut value: BigUint = BigUint::from(0u16);
    for i in (0..16) {
        value <<= 16;
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let compressed_ex1 = u8_32_array_to_u16_16_array(&ex1);
    ...
    let compressed_sk1 = u8_32_array_to_u16_16_array(&ex1);
    ...
    let compressed_sk2 = u8_32_array_to_u16_16_array(&ex2);
    ...
    for element in c.iter() {
        ...
        value <<= 16;
        value += BigUint::from(decrypted_ci);
