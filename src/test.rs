...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let sk1 = SecretKey::parse(&sk1_bytes).unwrap();
    let pk1 = PublicKey::from_secret_key(&sk1);

    let sk2 = SecretKey::parse(&sk2_bytes).unwrap();
    let pk2 = PublicKey::from_secret_key(&sk2);

    let pkz1 = PublicKey::combine( &[pk1, pk2]).unwrap();
    print!("PKz1 {:?}\n", pkz1.serialize());


    // Calculate the sum of private keys sk1 and sk2
    let sk_sum_bytes = add_arrayz(&sk1_bytes, &sk2_bytes);
    let sk = SecretKey::parse(&sk_sum_bytes).expect("Invalid private key");
    let pkz2 = PublicKey::from_secret_key(&sk);
    ...
    assert_eq!(
        msg,
        decrypt(&sk1.serialize(), &encrypt(&pk1.serialize(), msg).unwrap()).unwrap().as_slice()
    );
